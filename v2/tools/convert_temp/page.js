...
/* ================= 非线性换算（基准：摄氏度） ================= */
const NAMES = { c:'摄氏度', f:'华氏度', k:'开尔文', r:'兰氏度' };

function toBase(v, key) {
  switch (key) {
    case 'c': return v;
...
function convert(src) {
  ...
  const base = toBase(val, src);
  for (const k in inputs) { if (k === src) continue; inputs[k].value = fmt(fromBase(base, k)); }
}
