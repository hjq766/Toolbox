...
description: Refactor or create tools in the jqnest toolbox project (v2/tools/<slug>/). Use this skill when the user asks to refactor an existing tool to match the v2 design system, create a new tool, apply design tokens, reuse shared modules (sidebar.css / upload-zone.js / code-editor.js), or ensure a tool complies with project conventions (HTML skeleton, five-section page.js, data-* selectors, Lucide icons, no hardcoded values). Covers 8 tool categories (linear/nonlinear converters, image processors, code editors, text tools, realtime generators, database queries, watermark tools). Includes ready-to-copy templates for each category and validation scripts.
license: MIT
---

# jqnest Tool Builder

一站式 skill：改版旧工具 / 新增工具，严格遵循 jqnest v2 设计系统与开发规范。
...
- **项目路径**：活跃代码在 `v2/`，`legacy/` 不可修改（仅作迁移源参考）
- **工具单元**：每个工具 = `v2/tools/<slug>/index.html` + `page.js`（可选 `tool.css` ≤80 行）
- **注册表**：`v2/public/scripts/data/tools.js` — 新增工具**必改**此处，否则首页看不到
- **启动命令**：`cd v2 && python3 -m http.server 5173`
- **验证地址**：
...
```text
1. 读取 v2/tools/<slug>/ 的 index.html + page.js
2. 对号入座（见 reference/tool-types.md）判断属于 8 类中的哪类
...
```text
1. 设计 slug（英文小写 + 下划线）
2. 调用 scripts/scaffold.sh <slug> <type> 创建目录 + 文件 + 注册到 tools.js
...
- vendor 文件放工具目录 → 放 `v2/public/vendor/`
...
| 共享模块 API | `reference/shared-modules.md` |
...
# 验证骨架完整性
scripts/check_skeleton.sh v2/tools/<slug>/index.html

# 扫描硬编码反模式
scripts/check_tokens.sh v2/tools/<slug>/
...
| 场景 | 参考工具 |
|---|---|
| 换算器（线性） | `v2/tools/convert_length/` |
| 换算器（非线性） | `v2/tools/convert_temp/` |
| 图片单文件 canvas | `v2/tools/images_flip/` |
| 图片批量 + ZIP | `v2/tools/images_convert/` |
| 代码编辑器 | `v2/tools/code_json/` |
| 实时表单生成 | `v2/tools/meta/` |
| 本地数据库 + 侧栏 | `v2/tools/areacode/` |
| 水印（九宫格） | `v2/tools/watermark_images/` |
...
- 字号：`--text-xs` ~ `--text-5xl`
- 品牌色：`--color-brand` / `--color-brand-hover` / `--color-brand-soft`
