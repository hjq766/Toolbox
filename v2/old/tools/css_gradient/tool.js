...
function updateSVGButtonState() {
    const svgButton = document.getElementById('copySVG');
    if (!svgButton) return;

    if (currentType === 'conic' || isRepeating) {
        // 角度渐变或重复渐变时禁用SVG按钮
        svgButton.disabled = true;
        svgButton.classList.add('disabled', 'tooltip');

        let tooltipText = '';
        if (currentType === 'conic') {
            tooltipText = 'SVG不支持角度渐变，请使用线性或径向渐变';
        } else if (isRepeating) {
            tooltipText = 'SVG不支持重复渐变，请关闭重复模式';
        }

        svgButton.setAttribute('data-tooltip', tooltipText);
        svgButton.removeAttribute('title'); // 移除默认title
    } else {
        // 其他渐变类型启用SVG按钮
        svgButton.disabled = false;
        svgButton.classList.remove('disabled', 'tooltip');
        svgButton.removeAttribute('data-tooltip');
        svgButton.setAttribute('title', '复制SVG渐变代码');
    }
}
...
function initButtons() {
    ...
    document.getElementById('copySVG').addEventListener('click', (e) => {
        ...
        if (button.disabled || button.classList.contains('disabled')) {
            let message = '';
            if (currentType === 'conic') {
                message = 'SVG不支持角度渐变，请使用线性或径向渐变';
            } else if (isRepeating) {
                message = 'SVG不支持重复渐变，请关闭重复模式';
            } else {
                message = 'SVG功能暂不可用';
            }
...
function generateSVGGradient() {
    ...
    colorStops.forEach(stop => {
        ...
        const offset = stop.position / 100; // 转换为0-1范围
        gradientStops += `    <stop offset="${offset}" stop-color="${color}" stop-opacity="${opacity}" />\n`;
    });
    ...
    switch (currentType) {
        case 'linear':
            ...
            gradientElement = `  <linearGradient id="${gradientId}" x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" gradientUnits="objectBoundingBox">\n${gradientStops}  </linearGradient>`;
            break;

        case 'radial':
            // 径向渐变
            const cx = radialPositionX / 100;
            const cy = radialPositionY / 100;
            const r = radialSize / 100;

            if (currentShape === 'circle') {
                gradientElement = `  <radialGradient id="${gradientId}" cx="${cx}" cy="${cy}" r="${r}" gradientUnits="objectBoundingBox">\n${gradientStops}  </radialGradient>`;
            } else {
                // 椭圆形渐变
                gradientElement = `  <radialGradient id="${gradientId}" cx="${cx}" cy="${cy}" r="${r}" gradientUnits="objectBoundingBox" gradientTransform="scale(1, 0.6)">\n${gradientStops}  </radialGradient>`;
            }
            break;

        case 'conic':
            // SVG不支持角度渐变，这种情况下按钮应该被禁用
            gradientElement = `  <radialGradient id="${gradientId}" cx="0.5" cy="0.5" r="0.5" gradientUnits="objectBoundingBox">\n${gradientStops}  </radialGradient>`;
            break;
    }

    // 生成完整的SVG，使用viewBox确保缩放正确
    const svg = `<svg width="400" height="400" viewBox="0 0 400 400" xmlns="http://www.w3.org/2000/svg">
  <defs>
${gradientElement}
  </defs>
  <rect x="0" y="0" width="400" height="400" fill="url(#${gradientId})" />
</svg>`;
...
// 初始化精选渐变样式
function initPresetGradients() {
    // 检查是否已加载color.js中的gradients数据
    if (typeof gradients !== 'undefined' && gradients.length > 0) {
        gradientsData = gradients;
        console.log('成功加载渐变数据:', gradientsData);
    } else {
        console.error('无法加载渐变样式数据，请确保color.js文件已正确加载');
        return;
    }
    ...
    // 加载第一个分类的渐变
    const firstCategory = Object.keys(gradientsData[0])[0];
    loadPresetGradients(firstCategory);
}

// 动态生成分类标签
function generateCategoryTabs() {
    const categoriesContainer = document.getElementById('presetCategories');
    if (!categoriesContainer) return;
    
    // 清空现有内容
    categoriesContainer.innerHTML = '';
    
    // 检查是否有categoryNames数据
    const names = (typeof categoryNames !== 'undefined') ? categoryNames : {};
    
    // 遍历所有分类并创建标签
    gradientsData.forEach((categoryGroup, index) => {
        Object.keys(categoryGroup).forEach(categoryKey => {
            const categoryName = names[categoryKey] || categoryKey;
            ...
            // 第一个标签设为活跃状态
            if (index === 0 && Object.keys(categoryGroup)[0] === categoryKey) {
...
function initCategoryTabs() {
    ...
    categoryTabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            loadPresetGradients(category);
        });
    });
}

// 加载预设渐变
function loadPresetGradients(category) {
    ...
    // 找到对应分类的渐变数据
    const categoryData = gradientsData.find(item => item[category]);
    if (!categoryData) {
        console.error('找不到分类数据:', category);
        return;
    }

    const gradientList = categoryData[category];

    // 创建渐变项
    gradientList.forEach(([name, gradient]) => {
        const presetItem = document.createElement('div');
        presetItem.className = 'preset-item';
        presetItem.setAttribute('data-name', name);
        presetItem.style.background = gradient;
        presetItem.setAttribute('data-gradient', gradient);

        // 添加点击事件
        presetItem.addEventListener('click', () => {
            applyPresetGradient(gradient);
...
// 应用预设渐变
function applyPresetGradient(gradientCSS) {
    try {
        // 解析渐变CSS
        const gradient = parseCSSGradient(gradientCSS);
        if (!gradient) {
            showToast('无法解析该渐变样式', 'error');
            return;
        }
        ...
        // 更新UI
        updateUIFromGradient();

        // 重新初始化颜色节点和更新渐变
        initColorStops();
        updateGradient();

        showToast('已应用渐变样式', 'success');
    } catch (error) {
        console.error('应用预设渐变失败:', error);
        showToast('应用渐变样式失败', 'error');
    }
}

// 根据渐变数据更新UI
function updateUIFromGradient() {
    // 更新渐变类型按钮
    const typeButton = document.querySelector(`[data-type="${currentType}"]`);
    if (typeButton) {
    ...
    if (currentType === 'linear' || currentType === 'conic') {
        ...
        const customAngle = document.getElementById('customAngle');
        if (customAngle) {
            customAngle.style.display = directionButton === '[data-direction="custom"]' ? 'block' : 'none';
        }
    }
    ...
    if (currentType === 'radial') {
        ...
        const shapeButton = document.querySelector(`[data-shape="${currentShape}"]`);
        if (shapeButton) {
            document.querySelectorAll('#radialSettings .shape-tab').forEach(btn => btn.classList.remove('active'));
