...
const lwRange     = $('[data-opt="lineWidth"]');
const lwVal       = $('[data-val-lineWidth]');
...
function buildOption(data) {
  ...
  const series = data.slice(1).map(row => ({
    ...
    areaStyle: areaFill ? { opacity: 0.25 } : undefined,
...
// 节点 (toggle)
const symbolCheck = $('[data-opt="symbol"]');
on(symbolCheck, 'change', () => { showSymbol = symbolCheck.checked; updateChart(); });
...
/* ========== 13. 导出 ========== */
setupExportPanel(chart, '折线图.png', chartSize);
