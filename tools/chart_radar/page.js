...
function buildOption(data) {
  ...
  const indicators = data.slice(1).map(row => {
    const vals = data[0].slice(1).map((_, si) => Number(data.slice(1).map(r => r[si + 1])[data.slice(1).indexOf(row)]) || 0);
    return { name: String(row[0]) };
  });

  // 自动计算每个指标的最大值
  const maxVals = data.slice(1).map((row, ri) => {
    let max = 0;
    for (let si = 1; si < data[0].length; si++) {
      const v = Number(data[ri + 1][si]) || 0;
      if (v > max) max = v;
    }
    return max;
  });

  const radarIndicators = data.slice(1).map((row, ri) => ({
    name: String(row[0]),
    max: Math.ceil(maxVals[ri] * 1.2 / 100) * 100 || 100,
  }));

  const seriesData = data[0].slice(1).map((name, si) => ({
    name: String(name),
    value: data.slice(1).map(row => Number(row[si + 1]) || 0),
  }));
...
bindSeg('[data-shape]',      v => { shape = v; updateChart(); });
bindSeg('[data-area]',       v => { areaFill = v === 'on'; updateChart(); });
...
/* ========== 13. 导出 ========== */
setupExportPanel(chart, '雷达图.png', chartSize);
