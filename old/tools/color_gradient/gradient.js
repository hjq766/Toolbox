...
function generateTabs() {
    ...
    // 从gradients数据中提取所有分类
    const dataCategories = gradients.map(item => Object.keys(item)[0]);
    
    // 合并分类列表
    const categories = allCategories.concat(dataCategories);
    
    // 生成tab按钮HTML
    const tabsHTML = categories.map((category, index) => {
        const isActive = index === 0 ? 'active' : '';
        const displayName = categoryNames[category] || category;
        return `<button class="tab-btn ${isActive}" data-category="${category}">${displayName}</button>`;
    }).join('');
    
    DOM.toolsNav.innerHTML = tabsHTML;
}
